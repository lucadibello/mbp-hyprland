#!/bin/bash
# /* ---- 💫 https://github.com/JaKooLit 💫 ---- */
# This script for selecting wallpapers (SUPER W)

# WALLPAPERS PATH
terminal=kitty
wallDIR="$HOME/Pictures/wallpapers"
SCRIPTSDIR="$HOME/.config/hypr/scripts"
wallpaper_current="$HOME/.config/hypr/wallpaper_effects/.wallpaper_current"

# Directory for swaync
iDIR="$HOME/.config/swaync/images"
iDIRi="$HOME/.config/swaync/icons"

# Check if package bc exists
if ! command -v bc &>/dev/null; then
  notify-send -i "$iDIR/error.png" "bc missing" "Install package bc first"
  exit 1
fi

# Variables
rofi_theme="$HOME/.config/rofi/config-wallpaper.rasi"
focused_monitor=$(hyprctl monitors -j | jq -r '.[] | select(.focused) | .name')

# Ensure focused_monitor is detected
if [[ -z "$focused_monitor" ]]; then
  notify-send -i "$iDIR/error.png" "E-R-R-O-R" "Could not detect focused monitor"
  exit 1
fi

# Monitor details
scale_factor=$(hyprctl monitors -j | jq -r --arg mon "$focused_monitor" '.[] | select(.name == $mon) | .scale')
monitor_height=$(hyprctl monitors -j | jq -r --arg mon "$focused_monitor" '.[] | select(.name == $mon) | .height')

icon_size=$(echo "scale=1; ($monitor_height * 3) / ($scale_factor * 150)" | bc)
adjusted_icon_size=$(echo "$icon_size" | awk '{if ($1 < 15) $1 = 20; if ($1 > 25) $1 = 25; print $1}')
rofi_override="element-icon{size:${adjusted_icon_size}%;}"

# Category picker — uses first image in each directory as its icon
select_category() {
  printf ". all\x00icon\x1f%s\n" "$RANDOM_PIC"
  find -L "$wallDIR" -mindepth 1 -maxdepth 1 -type d | sort | while IFS= read -r dir; do
    first_img=$(find -L "$dir" -maxdepth 1 -type f \( \
      -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o \
      -iname "*.gif" -o -iname "*.bmp" -o -iname "*.tiff" -o -iname "*.webp" \) \
      -print -quit)
    if [[ -n "$first_img" ]]; then
      printf "%s\x00icon\x1f%s\n" "$(basename "$dir")" "$first_img"
    else
      printf "%s\n" "$(basename "$dir")"
    fi
  done
}

# Kill existing wallpaper daemons for video
kill_wallpaper_for_video() {
  pkill hyprpaper 2>/dev/null
  pkill mpvpaper 2>/dev/null
  pkill swaybg 2>/dev/null
}

# Kill existing wallpaper daemons for image (keep hyprpaper running)
kill_wallpaper_for_image() {
  pkill mpvpaper 2>/dev/null
  pkill swaybg 2>/dev/null
}

# Retrieve wallpapers (both images & videos)
mapfile -d '' PICS < <(find -L "${wallDIR}" -type f \( \
  -iname "*.jpg" -o -iname "*.jpeg" -o -iname "*.png" -o -iname "*.gif" -o \
  -iname "*.bmp" -o -iname "*.tiff" -o -iname "*.webp" -o \
  -iname "*.mp4" -o -iname "*.mkv" -o -iname "*.mov" -o -iname "*.webm" \) -print0)

RANDOM_PIC="${PICS[$((RANDOM % ${#PICS[@]}))]}"
RANDOM_PIC_NAME=". random"

# Rofi command
rofi_command="rofi -i -show -dmenu -config $rofi_theme -theme-str $rofi_override"

# Sorting Wallpapers — accepts a list of pic paths as arguments
menu() {
  local pics=("$@")
  IFS=$'\n' sorted_options=($(sort <<<"${pics[*]}"))

  printf "%s\x00icon\x1f%s\n" "$RANDOM_PIC_NAME" "$RANDOM_PIC"

  for pic_path in "${sorted_options[@]}"; do
    pic_name=$(basename "$pic_path")
    if [[ "$pic_name" =~ \.gif$ ]]; then
      cache_gif_image="$HOME/.cache/gif_preview/${pic_name}.png"
      if [[ ! -f "$cache_gif_image" ]]; then
        mkdir -p "$HOME/.cache/gif_preview"
        magick "$pic_path[0]" -resize 1920x1080 "$cache_gif_image"
      fi
      printf "%s\x00icon\x1f%s\n" "$pic_name" "$cache_gif_image"
    elif [[ "$pic_name" =~ \.(mp4|mkv|mov|webm|MP4|MKV|MOV|WEBM)$ ]]; then
      cache_preview_image="$HOME/.cache/video_preview/${pic_name}.png"
      if [[ ! -f "$cache_preview_image" ]]; then
        mkdir -p "$HOME/.cache/video_preview"
        ffmpeg -v error -y -i "$pic_path" -ss 00:00:01.000 -vframes 1 "$cache_preview_image"
      fi
      printf "%s\x00icon\x1f%s\n" "$pic_name" "$cache_preview_image"
    else
      printf "%s\x00icon\x1f%s\n" "$(echo "$pic_name" | cut -d. -f1)" "$pic_path"
    fi
  done
}

# Offer SDDM Simple Wallpaper Option (only for non-video wallpapers)
set_sddm_wallpaper() {
  sleep 1

  # Resolve SDDM themes directory (standard and NixOS path)
  local sddm_themes_dir=""
  if [ -d "/usr/share/sddm/themes" ]; then
    sddm_themes_dir="/usr/share/sddm/themes"
  elif [ -d "/run/current-system/sw/share/sddm/themes" ]; then
    sddm_themes_dir="/run/current-system/sw/share/sddm/themes"
  fi

  [ -z "$sddm_themes_dir" ] && return 0

  local sddm_simple="$sddm_themes_dir/simple_sddm_2"

  # Only prompt if theme exists and its Backgrounds directory is writable
  if [ -d "$sddm_simple" ] && [ -w "$sddm_simple/Backgrounds" ]; then

    # Check if yad is running to avoid multiple notifications
    if pidof yad >/dev/null; then
      killall yad
    fi

    if yad --info --text="Set current wallpaper as SDDM background?\n\nNOTE: This only applies to SIMPLE SDDM v2 Theme" \
      --text-align=left \
      --title="SDDM Background" \
      --timeout=5 \
      --timeout-indicator=right \
      --button="yes:0" \
      --button="no:1"; then

      # Check if terminal exists
      if ! command -v "$terminal" &>/dev/null; then
        notify-send -i "$iDIR/error.png" "Missing $terminal" "Install $terminal to enable setting of wallpaper background"
        exit 1
      fi

      exec "$SCRIPTSDIR/sddm_wallpaper.sh" --normal

    fi
  fi
}

modify_startup_config() {
  local selected_file="$1"
  local startup_config="$HOME/.config/hypr/UserConfigs/Startup_Apps.conf"

  if [[ "$selected_file" =~ \.(mp4|mkv|mov|webm)$ ]]; then
    # For video: disable hyprpaper, enable mpvpaper
    sed -i '/^\s*exec-once\s*=\s*hyprpaper\s*$/s/^/#/' "$startup_config"
    sed -i '/^\s*#\s*exec-once\s*=\s*mpvpaper\s*.*$/s/^#\s*//;' "$startup_config"
    selected_file="${selected_file/#$HOME/\$HOME}"
    sed -i "s|^\$livewallpaper=.*|\$livewallpaper=\"$selected_file\"|" "$startup_config"
  else
    # For image: ensure hyprpaper is enabled, disable mpvpaper
    sed -i '/^\s*#\s*exec-once\s*=\s*hyprpaper\s*$/s/^\s*#\s*//;' "$startup_config"
    sed -i '/^\s*exec-once\s*=\s*mpvpaper\s*.*$/s/^/#/' "$startup_config"
  fi
}

# Apply Image Wallpaper
apply_image_wallpaper() {
  local image_path="$1"
  local hyprpaper_conf="$HOME/.config/hypr/hyprpaper.conf"

  kill_wallpaper_for_image

  if ! pgrep -x "hyprpaper" >/dev/null; then
    hyprpaper &
    sleep 0.5
  fi

  # Get old wallpaper path before switching (for unloading)
  local old_wallpaper
  old_wallpaper=$(grep "^wallpaper" "$hyprpaper_conf" | head -1 | sed 's/wallpaper\s*=\s*[^,]*,//')

  hyprctl hyprpaper preload "$image_path"

  # Set on all connected monitors
  hyprctl monitors -j | jq -r '.[].name' | while read -r monitor; do
    hyprctl hyprpaper wallpaper "$monitor,$image_path"
  done

  # Unload previous wallpaper to free memory
  if [[ -n "$old_wallpaper" && "$old_wallpaper" != "$image_path" ]]; then
    sleep 0.3
    hyprctl hyprpaper unload "$old_wallpaper" 2>/dev/null || true
  fi

  # Persist selection in hyprpaper.conf
  printf "preload = %s\nwallpaper = ,%s\nsplash = false\n" "$image_path" "$image_path" > "$hyprpaper_conf"

  # Run additional scripts (pass the image path to avoid cache race conditions)
  "$SCRIPTSDIR/WallustSwww.sh" "$image_path"
  sleep 2
  "$SCRIPTSDIR/Refresh.sh"
  sleep 1

  set_sddm_wallpaper
}

apply_video_wallpaper() {
  local video_path="$1"

  # Check if mpvpaper is installed
  if ! command -v mpvpaper &>/dev/null; then
    notify-send -i "$iDIR/error.png" "E-R-R-O-R" "mpvpaper not found"
    return 1
  fi
  kill_wallpaper_for_video

  # Apply video wallpaper using mpvpaper
  mpvpaper '*' -o "load-scripts=no no-audio --loop" "$video_path" &
}

# Main function
main() {
  # Step 1: pick a category
  category=$(select_category | rofi -i -dmenu -p "󰉏  Category" -config "$rofi_theme" -theme-str "$rofi_override")
  [[ -z "$category" ]] && exit 0

  # Filter pics to the selected category
  if [[ "$category" == ". all" ]]; then
    filtered_pics=("${PICS[@]}")
    search_dir="$wallDIR"
  else
    filtered_pics=()
    for pic in "${PICS[@]}"; do
      [[ "$pic" == "$wallDIR/$category/"* ]] && filtered_pics+=("$pic")
    done
    search_dir="$wallDIR/$category"
  fi

  if [[ ${#filtered_pics[@]} -eq 0 ]]; then
    notify-send -i "$iDIR/error.png" "No wallpapers" "No wallpapers found in: $category"
    exit 1
  fi

  # Update random pic to be within the selected category
  RANDOM_PIC="${filtered_pics[$((RANDOM % ${#filtered_pics[@]}))]}"

  # Step 2: pick a wallpaper from the filtered list
  choice=$(menu "${filtered_pics[@]}" | $rofi_command)
  choice=$(echo "$choice" | xargs)
  RANDOM_PIC_NAME=$(echo "$RANDOM_PIC_NAME" | xargs)

  [[ -z "$choice" ]] && exit 0

  # Random pick uses the already-resolved full path
  if [[ "$choice" == "$RANDOM_PIC_NAME" ]]; then
    selected_file="$RANDOM_PIC"
  else
    choice_basename=$(basename "$choice" | sed 's/\(.*\)\.[^.]*$/\1/')
    selected_file=$(find "$search_dir" -iname "$choice_basename.*" -print -quit)
  fi

  if [[ -z "$selected_file" ]]; then
    echo "File not found. Selected choice: $choice"
    exit 1
  fi

  modify_startup_config "$selected_file"

  if [[ "$selected_file" =~ \.(mp4|mkv|mov|webm|MP4|MKV|MOV|WEBM)$ ]]; then
    apply_video_wallpaper "$selected_file"
  else
    apply_image_wallpaper "$selected_file"
  fi
}

# Check if rofi is already running
if pidof rofi >/dev/null; then
  pkill rofi
fi

main